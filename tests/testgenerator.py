...
class GenerateConstant(unittest.TestCase):
    ...
    def test_class_simple(self):
        ...
        result = Namespace(name = "foo",
                    classes = [Class(name        = "brabbel",
                       constructors = [Function(name   = "brabbel",
                                              symbol = "SYM_FIXME")]
                      )]).AdaSpecification()
    ...
    def test_class_elements(self):
        ...
        result =Namespace(name = "bar", namespaces=[Namespace(name="foo", classes=[
                Class(name        = "brabbel",
                       constructors = [Function(name   = "brabbel",
                                              symbol = "SYM_FIXME")],
                       members     = [Variable("field1", Type_Reference(Identifier(["int"]))),
    ...
    def test_class_elements_external_types(self):
        ...
        result = Namespace(name = "bar", namespaces=[Namespace(name="foo", classes=[
            Class(name        = "brabbel",
                       constructors = [Function(name = "brabbel", symbol="SYM_FIXME")],
                       members     = [Variable("field1", Type_Reference(Identifier(["bar", "baz", "my_type"]))),
    ...
    def test_class_functions_with_return_type(self):
        ...
        result = Namespace(name = "bar", namespaces=[Namespace(name="foo", classes=[
            Class(name        = "brabbel",
                       constructors  = [Function("brabbel", symbol="SYM_FIXME")],
                       members      = [Variable("field1", Type_Reference(Identifier(["bar", "baz", "my_type"]))),
                                       Variable("field2", Type_Reference(Identifier(["local_type"]))),
                                       Variable("field3", Type_Reference(Identifier(["bar", "foo", "blub", "some_type"])))],
                       functions    = [Function(name        = "do_something",
                                                symbol      = "this_function_has_a_funny_symbol",
                                                parameters  = [Variable("param1", Type_Reference(Identifier(["foo", "bar"]))),
    ...
    def test_namespace_with_constants(self):
        ...
        bar = open("tests/data/test_package_bar.txt", "r").read()
        foo = open("tests/data/test_package_bar_foo.txt", "r").read()
        result = Namespace(name = "bar", namespaces = [Namespace(name = "foo", namespaces = [
            Namespace(name      = "namespace",
                           constants = [Constant(name = "constant1", value = 5),
    ...
    def test_class_with_constants(self):
        ...
        result = Namespace(name = "bar", namespaces = [Namespace(name = "foo", classes = [
            Class(name        = "class",
                       constructors = [Function("create_me", symbol="symbol_404")],
                       constants   = [Constant(name = "constant1", value = 5),
                                      Constant(name = "constant2", value = 42)]
                      )])]).AdaSpecification()
        self.assertTrue(result[2] == expected, "Invalid class: >>>\n" + result[2] + "\n<<< expected: >>>\n" + expected + "\n<<<")
    ...
    def test_class_with_enum(self):
        ...
        result = Namespace(name = "bar", namespaces=[Namespace(name="foo", classes=[
            Class(name        = "class",
                       constructors = [Function("create_me", symbol="symbol_404")],
                       enums       = [Enum(name = "enum1", constants = [Constant("Elem11", 5),
                                                                                      Constant("Elem12", 6)]),
                                      Enum(name = "enum2", constants = [Constant("Elem21"),
                                                                                      Constant("Elem22"),
                                                                                      Constant("Elem23")])])])]).AdaSpecification()
        self.assertTrue(result[2] == expected, "Invalid class: >>>\n" + result[2] + "\n<<< expected: >>>\n" + expected + "\n<<<")
        self.assertTrue(result[1] == foo, "Invalid package: >>>\n" + result[1] + "\n<<< expected: >>>\n" + foo + "\n<<<")
        self.assertTrue(result[0] == bar, "Invalid package: >>>\n" + result[0] + "\n<<< expected: >>>\n" + bar + "\n<<<")
    ...
    def test_enum_representation(self):
        result = Enum(name      = "foo",
                      constants = [Constant(name = "elem1", value = 50),
    ...
    def test_class_multiple_constructors(self):
        ...
        result = Namespace(name = "foo", classes = [
            Class(name        = "brabbel",
                       constructors = [Function(name   = "brabbel",
                                              symbol = "SYM_FIXME"),
                                       Function(name = "brabbel",
                                              symbol = "SYM_FIXME",
                                              parameters = [Variable(name = "arg",
                                                  ctype = Type_Reference(name=Identifier(["integer"])))])]
                      )]).AdaSpecification()
